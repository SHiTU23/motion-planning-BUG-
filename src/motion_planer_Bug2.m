...
current_pose = start;
obstacle_id = 1;
object_with_one_intersection = 0;
number_of_OI_objects = 0;
for intersections = 1:2:size(closest_intersections,1)
    %%% first intersection
     if object_with_one_intersection == 0 %%% the object had 2 intersections
         intersections_ID = intersections;
     else
         intersections_ID = intersections-number_of_OI_objects;
     end
     
     obstacle_id = closest_intersections(intersections_ID,5);
     plot_line(current_pose, closest_intersections(intersections_ID,1:2));
     current_pose = closest_intersections(intersections_ID,1:2);
     
     %%% if there is only one intersection with an obstacle, it means it is
     %%% in the corner, so go to the next iteration
     next_intersection_objectID = closest_intersections(intersections_ID+1,5);
     if next_intersection_objectID == obstacle_id
     
         %%% nearest corner
         nearest_point = find_nearest_point(current_pose, env{obstacle_id}(closest_intersections(intersections_ID,3),:), env{obstacle_id}(closest_intersections(intersections_ID,4),:));
         ...
         env_copy{obstacle_id}(closest_intersections(intersections_ID,3),:) = 0;
         env_copy{obstacle_id}(closest_intersections(intersections_ID,4),:) = 0;
         for i = 1:4
         ...
         next_intersection = closest_intersections(intersections_ID+1,1:2);
         ...
         object_with_one_intersection = 1;
         number_of_OI_objects = number_of_OI_objects + 1;
     end
end
